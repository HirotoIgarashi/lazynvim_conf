...
return {
  {
    ...
    ft = "markdown", -- Markdownファイルを開いた時だけ読み込む
    cmd = { "Mtoc" }, -- :Mtoc コマンドでも読み込む
    opts = {
      -- 設定（空でもデフォルト設定で動作します）
      auto_update = true, -- ファイル保存時に目次を自動更新
      is_ordered = true, -- trueにすると数字付きリスト(1. 2.)になります
      toc_header = "## 目次", -- 目次のタイトルを指定
    },
  },
}
